...
#define ESP_PING_CHECK_OPTLEN(optlen, opttype) do { if ((optlen) < sizeof(opttype)) { return ESP_ERR_PING_INVALID_PARAMS; }}while(0)
	...
	typedef enum {
		...
		PING_TARGET_IS = 56   /**< target IP address is */
	} ping_target_id_t;
	...
	*      - ESP_ERR_PING_INVALID_PARAMS
	*/
	esp_err_t esp_ping_result(uint8_t IDPING, uint8_t res_val, uint16_t res_len, uint32_t res_time);

extern ping_option ping_option_info[PING_COUNT];
